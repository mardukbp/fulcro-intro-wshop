...
LESSON 2: This demonstrates _locality_ - only a component knows what data it needs and it declares that next to its body. It also demonstrates the simplicity of only caring about the Root - queries are propagated up so that Root's query will get data for the whole page.
...
2. It is a bit long and hard to read so open http://localhost:9630/inspect-latest[Shadow-cljs Inspect Latest] in a new tab of the same browser and then evaluate the `(tap> ...)` form marked _Exercise 4.2_. The value should appear in the Shadow Inspect; at the very bottom, **click _Pretty-Print_**
3. Now let's see how Fulcro fulfills that query from the client DB, using `fdn/db->tree` - evaluate the form marked _Exercise 4.3_ and observe the result in Shadow's Inspect Latest. First use _Pretty-Print_ on it then switch to _Browse_ - you can click on any line to "drill down" and you can use the buttons `<` and `<<` to go (all the way) back

LESSON: The Root query is turned into a props tree using the client DB. Shadow Inspect is a fine tool for looking at complex data.
...
Where does the current-list's list data come from? From the server, as we will see. But first, let's review what the frontend part of a Fulcro application looks like:
...
1. Make sure _Fulcro Inspect_ is open and reload the page
...
*^1^) Tip*: If you do not see the transaction then reload the browser tab. (This happens if you opened Fulcro Inspect after the data has already been loaded.)
 * How does the LOAD query differ from the Root query? Are they the same? No? Why?
 ...
 * As you have noticed, by default Pathom returns all attributes of a _nested entity_ if you do not declare what you want using a join
...
LESSON 2: The backend exposes all the data the frontend might care about through a single endpoint and the frontend asks for whatever subset of it that it wants. To expose a new piece of data just write a backend function that returns it and add it to the frontend query.

#### 7. Loading data from the REPL

In the previous exercise we have observed the _effect_ of the application loading its data. Now we will trigger the LOAD ourselves from the code, in the same manner that the application https://github.com/holyjak/fulcro-intro-wshop/blob/6a06c8881696ff2225be3502d79f0289054fb741/src/fulcro_todomvc/main.cljs#L13[does it in its startup code].
 ...
 * Notice that it only returns the uuid of the submitted transaction, which is executed _asynchronously_
...
2. Check that you can see the transaction in the _Transactions_ tab of Fulcro Inspect and explore the request and response in its _Network_ tab.
 ...
 * The frontend and backend "handlers" must be registered under the same symbol, which is normally derived from defmutation's name and the current namespace; since here the backend mutation is in a different ns than the frontend one, we override that with the `::pc/sym` config option
 * It updates the server-side "db" and returns an empty map (as we have seen in the Network tab)

LESSON: A mutation is data that can have a handler associated on the frontend and the backend. The frontend handler action updates the Client DB and the mutation is then sent asynchronously to the backend as an EQL transaction iff it is marked as remote. The backend handler gets the same parameters (plus the general pathom environment) and typically updates a data store and possibly returns data.

LESSON 2: Navigability - you can click through to the mutation's code from the place where you "transact" it. Locality - the code of the frontend and backend mutations can be next to each other. Productivity - adding a new local/remote side-effect is one defmutation away.
...
3. In `api.cljs`, find the `todo-check` mutation (you can Command-click / Control-click the "call" to go to its definition) and inside the `remote` section change [.line-through]`true` to `false` and _save the file_ (so that Shadow will hot-reload the code) and wait a few seconds. (Perhaps even check the browser's Console for the info log `shadow-cljs: load JS fulcro_todomvc/api.cljs`.)
...
5. (Bonus) At the top left of the _DB_ tab, click on the `<` next to the slider a few times - notice the item's attributes in the client DB revert to the previous state and you can use the circular arrow icon to reset the app and UI to that state (which creates a new _revision_ of the state). This way you can go back and forth in the history. (The UI used to get changed as you did this but that seems not to work for me anymore. This is reportedly a cool feature in theory but worse in practice due to side-effects such as I/O and lifecycle hooks.)
6. Reload the browser page. Notice that "Item 3" is no longer marked as completed (since it was only checked on the frontend and we have now replaced its data with the backend data)
...
9. Changes are performed by transacting a mutation, which is just a list consisting of a symbol and parameters - and it may have associated frontend and possibly backend "handlers" via the `defmutation` macros
...
TIP: If you run into trouble then check the <<Help>> section below
...
2. 🏁 Start the REPL for the exercises:
...
Use repeatedly `(hint <exercise number>)` (as long as there are any hints left for the exercise) to get useful tips when you get stuck.
...
If shadow-cljs fails because of a coding error then this error is reported in the browser. It may break live reloading, i.e. you might need to reload the page after you fix it.
